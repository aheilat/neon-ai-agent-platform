...
export default function IndependentWidget() {
  ...
  const send = async (event: FormEvent) => {
    ...
    if (!message || !agent || sending || handoffComplete) return;
  ...
  const submitHandoff = async (event: FormEvent) => {
    ...
    setError(null);
    try {
      const result = await publicWidgetRequest<{ contact: HandoffContact }>(`/api/public/agents/${agent.id}/handoff`, { method: "POST", body: JSON.stringify({ ...handoff, conversationId }) });
