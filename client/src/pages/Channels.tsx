...
import { WHATSAPP_QUICK_TEMPLATES } from "@shared/whatsappTemplates";
import { Check, CircleAlert, ClipboardCheck, Copy, Globe2, Instagram, Link2, Loader2, MessageCircle, PhoneCall, Radio, RefreshCw, Send, ShieldCheck, Sparkles } from "lucide-react";
...
export default function Channels() {
  ...
  const copyWhatsAppTemplate = async (template: typeof WHATSAPP_QUICK_TEMPLATES[number]) => {
    await navigator.clipboard?.writeText(template.body);
    toast.success(`تم نسخ قالب «${template.label}».`);
  };
  ...
  return <div className="mx-auto max-w-[1400px] space-y-7 pb-12" dir="rtl">
    ...
    <Card className="border-lime-300/20 bg-gradient-to-br from-lime-300/[0.06] to-[#0b1728]/90"><CardHeader className="flex flex-row items-start justify-between gap-4 border-b border-white/10 pb-5"><div><CardTitle className="text-base text-white">قوالب WhatsApp العربية الجاهزة</CardTitle><p className="mt-1 text-xs leading-5 text-slate-400">ابدأ من رسالة واضحة ثم عدّلها وفق خدمة العميل. هذه مسودات قابلة للنسخ وليست رسائل مرسلة تلقائياً.</p></div><ClipboardCheck className="h-5 w-5 text-lime-300" /></CardHeader><CardContent className="space-y-4 p-5"><div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">{WHATSAPP_QUICK_TEMPLATES.map(template => <div key={template.id} className="rounded-2xl border border-white/[0.08] bg-white/[0.025] p-4"><div className="flex items-center justify-between gap-3"><div><p className="text-sm font-semibold text-white">{template.label}</p><p className="mt-1 text-[11px] text-lime-200/80">{template.purpose}</p></div><Button onClick={() => copyWhatsAppTemplate(template)} variant="outline" className="h-8 rounded-lg border-white/10 bg-white/[0.04] px-2 text-xs text-white hover:bg-white/10"><Copy className="ml-1 h-3.5 w-3.5" /> نسخ</Button></div><p className="mt-3 text-xs leading-6 text-slate-300">{template.body}</p></div>)}</div><div className="rounded-xl border border-amber-300/20 bg-amber-300/[0.06] p-3 text-xs leading-6 text-amber-100"><ShieldCheck className="ml-2 inline h-4 w-4" /> للرسائل الاستباقية خارج نافذة خدمة WhatsApp، يجب اعتماد القالب في WhatsApp Manager واستخدام صياغته المعتمدة. لا تُرسل Neon هذه المسودات تلقائياً.</div></CardContent></Card>
