...
export default function IndependentOnboarding() {
  ...
  const analyze = async () => {
    ...
    try {
      ...
      setError(/timed out|timeout/i.test(message) ? "استغرق تحليل الموقع وقتاً أطول من المتوقع. تحقق من أن الرابط عام ثم أعد المحاولة." : message || "تعذر تحليل الموقع الآن.");
    }
  ...
  return <main dir="rtl" className="min-h-screen bg-[#f7f8fc] text-[#12172b]">
    ...
    <section className="mx-auto max-w-5xl px-4 py-8 sm:px-8 sm:py-12">
      ...
      {error && <div className="mx-auto mt-6 flex max-w-2xl flex-col gap-3 rounded-2xl border border-rose-200 bg-rose-50 p-4 text-sm text-rose-700"><p>{error}</p>{step === 2 && <Button type="button" variant="outline" onClick={() => void analyze()} disabled={busy} className="h-11 rounded-xl border-rose-300 bg-white text-rose-700 hover:bg-rose-100">{busy ? <Loader2 className="h-4 w-4 animate-spin" /> : "إعادة المحاولة"}</Button>}</div>}
    </section>
  </main>;
}
