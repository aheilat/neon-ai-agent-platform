...
export default function IndependentStaging() {
  ...
  const resetMetaSimulation = () => {
    if (!selectedAgent) return;
    try {
      window.localStorage.removeItem(mockMetaStorageKey(selectedAgent.id));
      setMockMetaConnection(null);
      setMockMetaError(null);
    } catch {
      setMockMetaError("تعذر إعادة ضبط المحاكاة. حاول مرة أخرى من هذا المتصفح.");
    }
  };
  ...
  const primaryAgentControlPanel = selectedAgent && location === "/settings" ? <section id="primary-agent-control" className="mb-6 scroll-mt-5 rounded-3xl border border-cyan-200/25 bg-gradient-to-l from-cyan-300/[0.10] to-lime-300/[0.06] p-5 sm:p-6">
    <div className="flex flex-wrap items-start justify-between gap-4"><div><div className="flex items-center gap-2"><Sparkles className="h-5 w-5 text-cyan-200" /><h2 className="text-lg font-bold">إدارة وكيلك الأساسي</h2></div><p className="mt-2 max-w-2xl text-sm leading-7 text-slate-300">عدّل شخصية الوكيل الأساسي وإعداداته من مكان واحد. هذه التغييرات تخص مساحة عملك والوكيل المحدد فقط.</p></div><span className="rounded-full border border-cyan-200/25 bg-cyan-300/10 px-3 py-1 text-xs font-bold text-cyan-100">{selectedAgent.name}</span></div>
    <div className="mt-5 grid gap-4 sm:grid-cols-2"><label className="block text-sm font-bold">اسم الوكيل<input value={profile.name} onChange={(event) => setProfile((current) => ({ ...current, name: event.target.value }))} className="mt-2 w-full rounded-xl border border-white/15 bg-slate-950/60 p-3 text-sm text-white outline-none ring-cyan-200/70 placeholder:text-slate-500 focus:ring-2" /></label><label className="block text-sm font-bold">وصف الوكيل<input value={profile.description ?? ""} onChange={(event) => setProfile((current) => ({ ...current, description: event.target.value || null }))} placeholder="مثال: يرد على عملاء الشركة" className="mt-2 w-full rounded-xl border border-white/15 bg-slate-950/60 p-3 text-sm text-white outline-none ring-cyan-200/70 placeholder:text-slate-500 focus:ring-2" /></label></div>
    <label className="mt-4 block text-sm font-bold">كيف يتحدث الوكيل؟<textarea value={profile.persona ?? ""} onChange={(event) => setProfile((current) => ({ ...current, persona: event.target.value || null }))} placeholder="اكتب الشخصية، الأسلوب، والحدود التي يجب أن يلتزم بها الوكيل" className="mt-2 min-h-32 w-full rounded-xl border border-white/15 bg-slate-950/60 p-3 text-sm leading-7 text-white outline-none ring-cyan-200/70 placeholder:text-slate-500 focus:ring-2" /></label>
    <div className="mt-4 grid gap-4 sm:grid-cols-3"><label className="block text-sm font-bold">لغة الرد<select value={profile.language} onChange={(event) => setProfile((current) => ({ ...current, language: event.target.value as IndependentAgentProfile["language"] }))} className="mt-2 w-full rounded-xl border border-white/15 bg-slate-950/60 p-3 text-sm text-white outline-none"><option value="bilingual">عربي + English</option><option value="ar">العربية</option><option value="en">English</option></select></label><label className="block text-sm font-bold">نبرة الرد<select value={profile.tone} onChange={(event) => setProfile((current) => ({ ...current, tone: event.target.value as IndependentAgentProfile["tone"] }))} className="mt-2 w-full rounded-xl border border-white/15 bg-slate-950/60 p-3 text-sm text-white outline-none"><option value="friendly">ودود</option><option value="professional">احترافي</option><option value="direct">مباشر</option></select></label><label className="block text-sm font-bold">حالة الوكيل<select value={profile.status} onChange={(event) => setProfile((current) => ({ ...current, status: event.target.value as IndependentAgentProfile["status"] }))} className="mt-2 w-full rounded-xl border border-white/15 bg-slate-950/60 p-3 text-sm text-white outline-none"><option value="active">نشط ويستقبل العملاء</option><option value="paused">متوقف مؤقتاً</option><option value="draft">مسودة</option></select></label></div>
    <div className="mt-5 flex flex-wrap items-center gap-3"><Button onClick={() => void saveAgentProfile()} disabled={savingProfile} className="bg-cyan-300 text-slate-950 hover:bg-cyan-200">{savingProfile ? <><Loader2 className="ml-2 h-4 w-4 animate-spin" />جارٍ حفظ الإعدادات…</> : <><Save className="ml-2 h-4 w-4" />حفظ إعدادات الوكيل</>}</Button><p className="text-xs leading-6 text-slate-400">لن يتغير الوكيل إلا بعد الضغط على حفظ.</p></div>
  </section> : null;
    ...
    <main data-independent-view={independentWorkspaceNavigation.some((item) => item.href === location) ? location.slice(1) : "workspace"} className="independent-shell min-h-screen px-3 py-4 sm:px-8 sm:py-8" dir="rtl">
      <div className="independent-container mx-auto max-w-6xl">
        ...
        <header className="independent-header flex flex-wrap items-center justify-between gap-4 rounded-[2rem] border px-4 py-4 shadow-sm sm:px-6">
          ...
          <div className="flex flex-wrap items-center gap-2"><a href="/profile" className="inline-flex h-10 items-center rounded-xl border border-cyan-200/20 px-3 text-sm font-bold text-cyan-100 transition hover:bg-cyan-200/10">ملفي الشخصي</a><Button variant="outline" onClick={() => void loadWorkspace()} disabled={loading} className="border-white/15 text-white hover:bg-white/10"><RefreshCw className="ml-2 h-4 w-4" />تحديث</Button><Button variant="outline" onClick={() => void signOut()} className="border-white/15 text-white hover:bg-white/10"><LogOut className="ml-2 h-4 w-4" />خروج</Button></div>
        </header>
