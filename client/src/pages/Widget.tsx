...
export default function Widget() {
  ...
  {selectedAttachment && (
    <div className="mb-2 flex items-center gap-2 rounded-lg bg-white/[0.08] px-3 py-1.5 text-xs text-slate-200 w-fit">
      {selectedAttachment.type === "image" ? <ImageIcon className="h-3.5 w-3.5 text-cyan-300" /> : selectedAttachment.type === "audio" ? <Mic className="h-3.5 w-3.5 text-lime-300 animate-pulse" /> : <Paperclip className="h-3.5 w-3.5 text-slate-300" />}
      <span className="max-w-[180px] truncate">{selectedAttachment.name}</span>
      <button type="button" onClick={() => setSelectedAttachment(null)} className="text-slate-400 hover:text-white"><X className="h-3 w-3" /></button>
    </div>
  )}
