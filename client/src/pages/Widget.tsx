...
import { Bot, ChevronDown, Image as ImageIcon, Loader2, MessageCircle, Mic, Paperclip, Send, Sparkles, Square, Star, UserRound, X } from "lucide-react";
import { FormEvent, useRef, useState } from "react";
import { useRoute } from "wouter";

type ChatMessage = { sender: "customer" | "agent"; content: string };
type ClosingStage = "idle" | "ask-more" | "confirm-close" | "closed";
...
export default function Widget() {
  ...
  const appendAgentMessage = (content: string) => setMessages(current => [...current, { sender: "agent", content }]);

  const replyMutation = trpc.chat.publicReply.useMutation({
    onSuccess: result => {
      setConversationId(result.conversationId ?? undefined);
      if (result.content) appendAgentMessage(result.content);
      if (result.escalated || result.handoff) {
        setIsHandoffActive(true);
      } else {
        setClosingStage("ask-more");
        appendAgentMessage("هل تحتاج إلى مساعدة أخرى؟");
      }
    },
    onError: () => appendAgentMessage("أعتذر، حصل خلل بسيط. جرّب مرة ثانية."),
  });

  const handoffContactMutation = trpc.chat.publicHandoffContact.useMutation({
    onSuccess: () => {
  ...
  const closeConversationMutation = trpc.chat.publicCloseConversation.useMutation({
    onSuccess: () => {
      setClosingStage("closed");
      appendAgentMessage("تم إنهاء المحادثة. نرجو تقييم تجربتك لمساعدتنا على التحسين.");
    },
    onError: () => appendAgentMessage("تعذر إنهاء المحادثة الآن. يمكنك المحاولة بعد قليل."),
  });

  const ratingMutation = trpc.chat.publicRateConversation.useMutation({
    onSuccess: () => appendAgentMessage("شكراً لتقييمك. يسعدنا خدمتك دائماً."),
  });
  ...
  const startRecording = async () => {
    try {
      ...
      recorder.onstop = () => {
        ...
        setSelectedAttachment({ name: "رسالة صوتية مسجلة.webm", type: "audio", url: audioUrl });
  ...
  const send = (event: FormEvent) => {
    ...
    if ((!value && !selectedAttachment) || !agentId || replyMutation.isPending || isHandoffActive || closingStage !== "idle") return;
    const attachmentPrefix = selectedAttachment?.type === "image" ? "[مرفق صورة]" : selectedAttachment?.type === "audio" ? "[مرفق رسالة صوتية]" : selectedAttachment ? "[مرفق ملف]" : "";
    const finalContent = selectedAttachment ? `${value ? `${value} ` : ""}${attachmentPrefix} (${selectedAttachment.name})` : value;
    setMessages(current => [...current, { sender: "customer", content: finalContent }]);
    setMessage("");
    setSelectedAttachment(undefined);
    replyMutation.mutate({ agentId, conversationId, message: finalContent });
  };
  ...
  const saveHandoffContact = (event: FormEvent) => {
    ...
    handoffContactMutation.mutate({ agentId, conversationId, name: lead.name.trim(), phone: lead.phone.trim(), email: lead.email.trim() });
  };
  ...
  const continueConversation = () => {
    ...
    appendAgentMessage("بالتأكيد، كيف يمكنني مساعدتك؟");
  };

  const askToClose = () => {
    setClosingStage("confirm-close");
    appendAgentMessage("هل تريد إنهاء المحادثة الآن؟");
  };

  const confirmClose = () => {
    if (!conversationId) return;
    closeConversationMutation.mutate({ agentId, conversationId });
  };

  const submitRating = (value: number) => {
    if (!conversationId || ratingMutation.isPending || rating) return;
    setRating(value);
    ratingMutation.mutate({ agentId, conversationId, rating: value });
  };
