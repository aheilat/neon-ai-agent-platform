...
export default function Widget() {
  ...
  const replyMutation = trpc.chat.publicReply.useMutation({
    onSuccess: result => {
      ...
      if (result.content) setMessages(current => [...current, { sender: "agent", content: result.content }]);
  ...
  const send = (event: FormEvent) => {
    ...
    if ((!value && !selectedAttachment) || !agentId || replyMutation.isPending || isHandoffActive) return;
  ...
  <form onSubmit={send} className="flex items-center gap-2 bg-white/[0.05] border border-white/10 rounded-2xl px-3 py-1.5 focus-within:ring-1 focus-within:ring-cyan-300">
    ...
    <button type="button" disabled={isHandoffActive} onClick={() => fileInputRef.current?.click()} className="text-slate-400 hover:text-cyan-300 transition-colors p-1 disabled:cursor-not-allowed disabled:opacity-40" title="إرفاق ملف أو صورة">
    ...
    {isRecording ? (
      ...
      <button type="button" disabled={isHandoffActive} onClick={startRecording} className="text-slate-400 hover:text-lime-300 transition-colors p-1 disabled:cursor-not-allowed disabled:opacity-40" title="تسجيل رسالة صوتية">
        <Mic className="h-4 w-4" />
      </button>
    )}
    <Input value={message} disabled={isHandoffActive} onChange={e => setMessage(e.target.value)} placeholder={isHandoffActive ? "تم تحويل المحادثة إلى الفريق" : isRecording ? "جاري التسجيل الصوتي..." : "اكتب استفسارك هنا..."} className="h-9 border-0 bg-transparent text-sm text-white placeholder:text-slate-500 focus-visible:ring-0 px-1 shadow-none disabled:cursor-not-allowed disabled:opacity-50" />
    <Button type="submit" disabled={isHandoffActive || (!message.trim() && !selectedAttachment) || replyMutation.isPending} className="h-9 w-9 shrink-0 rounded-xl bg-lime-300 p-0 text-slate-950 hover:bg-lime-200">
